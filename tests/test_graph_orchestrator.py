...
def test_analyze_with_graph_combines_sources(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "padv.orchestrator.graphs.discover_source_candidates",
        lambda repo_root, cfg: ([_mk_candidate("source-1", ["source"])], [_mk_evidence("source-1", "source::sql")]),
    )
    monkeypatch.setattr(
        "padv.orchestrator.graphs.discover_candidates",
        lambda repo_root, config: ([_mk_candidate("joern-1", ["joern"])], [_mk_evidence("joern-1", "joern::sql")]),
    )
    monkeypatch.setattr(
        "padv.orchestrator.graphs.discover_scip_candidates_safe",
        lambda repo_root, config: ([], [], [], None),
    )
    ...
    assert set(candidates[0].provenance) == {"source", "joern"}
...
def test_run_with_graph_uses_validation_node(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    candidate = _mk_candidate("cand-00001", ["source"])
    static = _mk_evidence("cand-00001", "source::sql")

    monkeypatch.setattr(
        "padv.orchestrator.graphs.discover_source_candidates",
        lambda repo_root, cfg: ([candidate], [static]),
    )
    monkeypatch.setattr("padv.orchestrator.graphs.discover_candidates", lambda repo_root, config: ([], []))
    monkeypatch.setattr("padv.orchestrator.graphs.discover_scip_candidates_safe", lambda *args, **kwargs: ([], [], [], None))
    ...
    monkeypatch.setattr(
        "padv.orchestrator.graphs.make_validation_plan_with_deepagents",
        lambda candidate, config, repo_root=None, session=None: (
            ValidationPlan(
    ...
    assert summary.discovery_trace["source_count"] == 1
...
def test_web_seed_urls_are_normalized_and_artifact_is_persisted(
    ...
    monkeypatch.setattr("padv.orchestrator.graphs.discover_source_candidates", lambda *_args, **_kwargs: ([], []))
    monkeypatch.setattr("padv.orchestrator.graphs.discover_candidates", lambda *_args, **_kwargs: ([], []))
    monkeypatch.setattr("padv.orchestrator.graphs.discover_scip_candidates_safe", lambda *_args, **_kwargs: ([], [], [], None))
...
def test_auth_setup_resolves_auth_precondition(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    cand = _mk_candidate("source-auth-1", ["source"])
    cand.preconditions = ["auth-state-known"]
    monkeypatch.setattr("padv.orchestrator.graphs.discover_source_candidates", lambda *_args, **_kwargs: ([cand], [_mk_evidence(cand.candidate_id, "source::sql")]))
    monkeypatch.setattr("padv.orchestrator.graphs.discover_candidates", lambda *_args, **_kwargs: ([], []))
    monkeypatch.setattr("padv.orchestrator.graphs.discover_scip_candidates_safe", lambda *_args, **_kwargs: ([], [], [], None))
